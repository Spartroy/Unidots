    ...
    const fetchOrders = async () => {
      try {
        ...
        if (filter === 'all') {
          ...
          // We don't have a direct API endpoint for assigned orders,
          // so we'll fetch all and filter client-side for this specific case
          // The server already filters for the employee's orders
          endpoint += '&status[ne]=Cancelled';
        }
