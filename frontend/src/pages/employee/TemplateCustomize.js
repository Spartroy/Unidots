...
const TemplateCustomize = () => {
  ...
  const drawPackageShape = (ctx, subCategory, scale) => {
    ...
    switch (subCategory) {
      case 'Milk Carton':
        drawMilkCartonShape(ctx, width, height, scale);
        break;
      case 'Cereal Box':
        drawCerealBoxShape(ctx, width, height, scale);
        break;
      case 'Rice Package':
  ...
  const drawMilkCartonShape = (ctx, width, height, scale) => {
    ...
    ctx.fillRect(0, height * 0.1, width * 0.15, height * 0.8);
    ctx.fillRect(width * 0.85, height * 0.1, width * 0.15, height * 0.8);
    
    // Outline
    ctx.strokeStyle = '#2D3748';
    ctx.lineWidth = 2;
    ctx.strokeRect(0, height * 0.1, width, height * 0.8);
  ...
  const drawCerealBoxShape = (ctx, width, height, scale) => {
    const bgColor = customizations['background-color'] || '#FFFFFF';
    const primaryColor = customizations['primary-color'] || '#E74C3C';
    ...
    ctx.fillStyle = adjustBrightness(bgColor, 5);
    ctx.fillRect(width * 0.1, height * 0.05, width * 0.8, height * 0.9);
    
    // Side panels (3D effect)
    ctx.fillStyle = adjustBrightness(bgColor, -15);
    ctx.fillRect(0, 0, width * 0.1, height);
    ctx.fillRect(width * 0.9, 0, width * 0.1, height);
    
    // Top accent strip
    ctx.fillStyle = primaryColor;
    ctx.fillRect(width * 0.1, height * 0.05, width * 0.8, height * 0.08);
    
    // Bottom accent strip
    ctx.fillRect(width * 0.1, height * 0.87, width * 0.8, height * 0.08);
    
    // Outline
    ctx.strokeStyle = '#2D3748';
    ctx.lineWidth = 2;
    ctx.strokeRect(0, 0, width, height);
    ctx.strokeRect(width * 0.1, height * 0.05, width * 0.8, height * 0.9);
  };
  ...
  const drawImageElement = (ctx, element, x, y, scale) => {
    ...
    // Completely removed any border drawing - no dotted lines in downloads
  };
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center space-x-4">
            <div className="flex items-center space-x-2 text-sm text-gray-500">
              ...
              <span>Est. {customizationOptions.estimatedTime} min</span>
            </div>
            
            {/* Submit to Order Button */}
            <button
              onClick={() => setShowOrderAssignment(true)}
          ...
          <div className="bg-white rounded-lg shadow-lg p-6">
            ...
            <div className="space-y-2 text-xs text-gray-600">
              ...
              <p className="flex items-center">
                ...
                Real package shapes with professional design
              </p>
            </div>
