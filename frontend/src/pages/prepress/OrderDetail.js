    ...
    try {
      ...
      if (subProcess === 'washout' && newStatus === 'Completed' && order.specifications?.dimensions) {
        try {
          ...
          toast.success(`${subProcess} completed! Acid solution consumption recorded: ${(totalAreaM2 * 10).toFixed(2)}L`);
        } catch (acidError) {
          console.error('Error recording acid solution usage:', acidError);
          toast.warning(`${subProcess} completed, but failed to record acid solution usage`);
        }
      } else {
      ...
      <div className="bg-white shadow rounded-lg overflow-hidden">
              ...
              <div>
                ...
                <div className="mt-1 text-sm text-gray-900">
                  {order.client?.phone || order.customer?.phone ? (
                    <a 
                      href={`tel:${order.client?.phone || order.customer?.phone}`} 
                      className="text-primary-600 hover:text-primary-800"
                    >
                      {order.client?.phone || order.customer?.phone}
                    </a>
            ...
            <div className="bg-white border border-gray-200 rounded-md overflow-hidden mb-6">
              ...
              <div className="p-4">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-x-6 gap-y-4">
                  ...
                  <div>
                    <div className="text-sm font-medium text-gray-500">Acid Solution Needed</div>
                ...
                <div className="mt-4 p-3 bg-gray-50 rounded-md">
                  <div className="text-xs text-gray-600">
                    ...
                    <p><strong>Note:</strong> 70% of solution is recycled, actual consumption is lower</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="border-t border-gray-200 px-4 py-4 sm:px-6 bg-gray-50">
          ...
          Back to Orders
            ...
            {/* Conditionally show complete prepress button */}
            {order.status === 'In Prepress' && allProcessesCompleted() && (
              <button
                onClick={handleMarkPrepressCompleted}
                disabled={updating}
                className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500"
              >
                {updating ? 'Updating...' : 'Complete Prepress Stage'}
              </button>
            )}
          </div>
        </div>
