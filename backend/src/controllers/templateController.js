...
// @desc    Get popular templates dashboard
// @route   GET /api/templates/dashboard
// @access  Private (Employees)
const getTemplateDashboard = asyncHandler(async (req, res) => {
  const employeeId = req.user.id;

  // Get recently used templates by this employee
  const recentlyUsed = await Order.find({
    assignedTo: employeeId,
    'history.templateUsage': { $exists: true }
  })
  .populate('history.templateUsage.templateId')
  .sort({ updatedAt: -1 })
  .limit(5);

  // Get most popular templates
  const popular = await Template.find({ isActive: true })
    .populate('previewImage', 'fileUrl')
    .sort({ 'stats.usageCount': -1 })
    .limit(8);

  // Get quick-start templates (beginner level)
  const quickStart = await Template.find({ 
    isActive: true, 
    difficulty: 'Beginner' 
  })
  .populate('previewImage', 'fileUrl')
  ...
  const byCategory = await Template.aggregate([
    ...
    { $group: { 
      _id: '$category', 
      count: { $sum: 1 },
      avgTime: { $avg: '$estimatedTimeMinutes' },
  ...
  res.json({
    recentlyUsed: recentlyUsed.map(order => ({
      orderId: order._id,
      orderNumber: order.orderNumber,
      template: order.history.find(h => h.templateUsage)?.templateUsage,
    ...
    byCategory,
    stats: {
      totalTemplates: await Template.countDocuments({ isActive: true }),
      userUsageCount: recentlyUsed.length
    }
  });
});
